...
describe("completion publisher", () => {
  it("uses the durable outbox ID as the JetStream dedupe ID", async () => {
    ...
    const client = { publish, close: async () => undefined } as UnsafeProductionJetStream
    const result = await Effect.runPromise(makeCompletionPublisher(client)(message))
    expect(result.duplicate).toBe(true)
    expect(publish).toHaveBeenCalledWith(message.subject, message.payload, message.messageId)
  })
  ...
  it("redacts SDK failures", async () => {
    ...
    const exit = await Effect.runPromiseExit(makeCompletionPublisher(client)(message))
