...
import { agentAuditMemoryRepository } from "./repository.js"
...
describe("sqlite agent audit and memory repository", () => {
  it("records an owner-scoped run and atomically sequences public events", async () => {
    ...
    const result = await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          const repository = yield* agentAuditMemoryRepository(
...
/** 監査記録はジョブに紐づくため、所有関係の確認に足るジョブを1件用意する。 */
const createDatabase = () => {
  const directory = mkdtempSync(join(tmpdir(), "production-agent-audit-"))
  directories.push(directory)
  const handle = openProductionDatabaseUnsafe(
