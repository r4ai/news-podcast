...
import { agentAuditMemoryRepository } from "../persistence/agent-audit/repository.js"
import { makeAgentAuditRpcHandler } from "./agent-audit.js"
...
describe("agent audit RPC", () => {
  it("lists only actor-owned instances and rejects reasoning memory payloads", async () => {
    ...
    await Effect.runPromise(
      Effect.scoped(
        Effect.gen(function* () {
          const repository = yield* agentAuditMemoryRepository(database)
