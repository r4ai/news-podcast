...
import { startEpisodeLibraryProcess } from "./process.js"

describe("episode-library process lifecycle", () => {
  it.each(["SIGINT", "SIGTERM"] as const)(
    "interrupts on %s, releases resources, flushes telemetry, and exits once",
    async (signal) => {
      const events: string[] = []
      const listeners = new Map<string, () => void>()
      const controller = startEpisodeLibraryProcess(
        Effect.scoped(
