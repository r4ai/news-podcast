...
GitHub Actionsのworkflowは、完全SHA固定、7日間のminimum release age、actionlint、zizmor、Gitleaks、`pnpm audit --config.ignore-pnpmfile=true --audit-level=high`で検査する。PR用のセキュリティworkflowはPRコードをcheckout・実行しない（pnpmfile・package lifecycle scriptも実行しない）。
