...
- 下端に浮く案内（回線切れ・生成キュー）の逃げ先は、本文の確保とは**別の変数**（`--player-notice-h`）で持つ。板は開くと上へ伸びるが、本文の確保（`--player-h`）は増やさない（増やすと開閉のたびに本文が跳ねる）。案内は板より手前に浮くので、確保と同じ値で置くと開いた段の速度・音量の行をちょうど覆う（実測: 通知615..644が失敗の行606..651を、通知691..720が速度select689..721を覆った）。開いた段には高さの上限を置き、逃げ先が有限に決まるようにする。
