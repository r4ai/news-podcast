...
- **開くための専用ボタンは置かない**。板の押せない場所はどこでも開き、キーボードからは題名がその役を兼ねる。常設の行は幅が足りず、ボタンを1つ足すだけで最も長く取りたい題名が数文字ぶん削れる。押せるもの（`a,button,input,select,textarea,[role=slider]`）の上で始まった操作と、文字を選んでいる最中は開かない。
