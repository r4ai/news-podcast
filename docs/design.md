...
AI記事補完のキュー、結果、タグ、日次AI試行枠はすべてowner単位である。workerはownerごとに`CONTENT_ENRICH_DAILY_LIMIT`の使用量を読み、枯渇したownerだけをskipして次のownerを処理する。タグ語彙はownerごとに100件を上限とし、作成・候補昇格・AI入力で同じ契約を守る。本文・語彙・profile・入力schemaを検証した後、provider送信の直前にlive leaseと残枠を同じtransactionで検査し、`(owner_id, UTC local_date)`へ有料試行を予約する。検証に失敗した項目は送信前に終端させ、別ownerの処理とRPCを継続する。OpenAI adapterは内部retryをせず、1予約を1 HTTP requestへ固定する。成功だけでなく429、timeout、不正応答も1試行を消費する一方、送信前失敗と期限切れleaseは消費しない。日次枠resetはContent KnowledgeのRPC境界で既定拒否し、非productionでserver flagを明示した場合だけactor自身のowner枠へ許可する。予約・枯渇とresetの成功・拒否を低cardinality metricへ記録する。所有境界は[ADR-0063](adr/0063-scope-enrichment-daily-budget-by-owner.md)、試行契約は[ADR-0084](adr/0084-reserve-paid-enrichment-attempts.md)、reset認可は[ADR-0076](adr/0076-fail-closed-enrichment-budget-reset.md)を正本とする。
...
- 一覧はopaque cursor、`limit` 1..100、安定順序、filterに束縛する。`totalCount`は初期契約に入れない。
  ...
  - 手動生成の候補は初回・検索・追加ページとも`sort=newest`（新着順）で取得し、アーカイブ済みの記事だけを表示する。「上から一括選択」は表示済みの先頭から最大20件を選ぶ。現行APIは`newest` / `oldest`のみ対応し、スコアの有無は順序へ影響しない。推薦順の提供範囲と旧仕様の変更は[ADR-0098](adr/0098-align-manual-picker-with-supported-order.md)を正本とする。
...
- 手動archive `POST /v1/me/articles/{articleId}/archive` はSQLite専用キューへの受付を `202 + jobId/status` で返す。`GET /v1/me/articles/{articleId}/archive/{jobId}` でownerと記事の権限を再検証して状態を取得する。captureは共有RPCと別workerで実行し、同じowner/articleのactive要求は再利用する。全体32件・owner 2件、毎分全体60件・owner 4件、実行は全体1件、受付から120秒・実行30秒を上限とする。詳細は[ADR-0093](adr/0093-isolate-manual-archive-behind-durable-admission.md)。
...
- 視覚方針は装飾を増やさないneutral UIとする。白い背景、意味のある境界線、既存のsemantic color、控えめな角丸だけで階層を表し、glow、装飾illustration、不要なbadgeやshadowを追加しない。
- 面が本文の**上に載る**関係は、glassで表す。素材（滲み量・透過・縁・影）は`--glass-*`のtokenと`@utility glass-surface`が1箇所で持ち、面ごとに色や影を書かない。透過できない環境と、OSで透過を減らしている利用者（`prefers-reduced-transparency: reduce`）には不透明の面へ落とす。これは見た目を落とした代替ではなく、同じ階層を成立させる正規の値として持つ（[ADR-0099](adr/0099-layered-glass-player-surface.md)）。
- 色で意味を作る場所は限る。今それに当たるのは再生バーの番組タイルだけで、`Episode`契約に画像が無いため`artworkHue(episodeId)`から色相を導く。同じ番組は何度開いても同じ色になり、読み上げには渡さない（題名が常に併記される）。色は目印であって情報ではない。
...
- 記事の既読は「開いた瞬間」ではなく「離れたタイミング」(別記事への切り替え・一覧へ戻る・ページ遷移・タブを閉じる)で反映する。開いている間は一覧でも未読表示のまま保つ。既読記事のリーダーには「未読に戻す」を操作列へ出し、タッチ・ポインターと`u` shortcutが同じactionを呼ぶ。
...
- 行内の操作(保存など)をhoverだけで出さない。タッチとキーボードから到達できなくなるため、フォーカス時と選択済み状態では常に見せる。記事行の未読/既読は色や太さだけにせず、小さな文字ラベルでも示す。読み上げでは同じ状態を行を開くbuttonのaccessible descriptionへ1回だけ加え、題名・媒体・時刻のaccessible nameを保ったまま判断できるようにする。
...
- 再生バーは画面幅いっぱいの帯ではなく、下端から浮かせた**1枚のglassの板**にする。モバイルでは下部ナビの上へ置く。本文末尾が板に隠れないよう、確保する高さは`--player-h`が1箇所で宣言し、板がDOMに在るかどうか(`:has()`)で決める。有無をstateで配ると、鳴らし始めた瞬間に画面全体が描き直される。
- 2ペインのページ（記事・ライブラリ）は、本文末尾に再生バーの分を空けない。あちらは画面の高さいっぱいの枠を自分で組み、その中で独立にスクロールする。末尾を空けると、枠の下に板1枚ぶんの何も無い帯ができて内容が画面の下まで届かない。板は透ける面として**内容の上に浮く**ので退ける必要がなく、最後の行が隠れないための余白はスクロールする側が持つ。
- 板は**画面の中央**へ置く（サイドバーを含めた中央）。主領域の中央に置くと、板だけが画面全体の中で右へずれて見える。左右の余白を「中央寄せに必要な量」と「サイドバー幅＋1rem」の大きい方に揃えることで、広い画面では画面中央、狭い画面ではサイドバーの右隣まで、どちらも左右対称に保てる。`lg`より下は両者が近すぎて板が潰れるので、主領域の中で収める。
- 下端に浮く案内（回線切れ・生成キュー）の逃げ先は、本文の確保とは**別の変数**（`--player-notice-h`）で持ち、確保へ**積む**形（`calc(var(--player-h)+13rem)`）で書く。定数で置くと、失敗の行が出て確保が厚くなった分を取りこぼす。板は開くと上へ伸びるが、本文の確保（`--player-h`）は増やさない（増やすと開閉のたびに本文が跳ねる）。案内は板より手前に浮くので、確保と同じ値で置くと開いた段の速度・音量の行をちょうど覆う（実測: 通知615..644が失敗の行606..651を、通知691..720が速度select689..721を覆った）。開いた段には高さの上限を置き、逃げ先が有限に決まるようにする。
- サイドバーは再生バーの高さを引かない。板はサイドバーの右隣までしか来ないので、引くとログアウトなどの末尾の操作だけが、何にも隠されていないまま1段浮いて止まる。
- 板は「常設1行」と「開いた中身」に分ける。常設は**絵・題名・目盛り・15秒戻し/再生/30秒送り・閉じる**を基本とし、**64rem以上では速度選択と音量Popoverも近道として残す**。全幅で大きい目盛り・速度・音量・原稿への道へ展開後に到達できる。常時2段にすると本文が2行ぶん削られる。常設操作やそのportalが幅変更で消えるとき、そこにfocusがあれば題名へ退避する（[ADR-0103](adr/0103-keep-wide-player-shortcuts-with-focus-handoff.md)）。
- **開くための専用ボタンは置かない**。板の押せない場所はどこでも開き、キーボードからは題名がその役を兼ねる。常設の行は幅が足りず、ボタンを1つ足すだけで最も長く取りたい題名が数文字ぶん削れる。押せるもの（`a,button,input,select,textarea,[role=slider]`）の上で始まった操作と、文字を選んでいる最中は開かない。展開中の余白では開く操作を始めず、畳むときは未完了の押下も破棄する。押したままEscapeで畳んだ後、指を離しても開き直さない。
- portalで外へ出た面の中の操作は、**DOMの中に在るかどうか**で切る（`currentTarget.contains(target)`）。Reactの出来事はportalを跨いで上がってくるので、名札（`role`）だけで切ると漏れる。実測では、速度の候補（`option`）を選ぶだけで板が開き、その拍子に候補を抱えた列ごと消えて選択まで失われていた。
- 下端から立ち上がる面（Drawer）の掴み代は、**引き下げて閉じられるようにする**。掴み代を描くだけで手を付けないと、引けそうに見えて引けない面になる。押しても閉じるようにし、動きと終わりは窓で受ける。所有中の指だけを処理し、キャンセル・Escape・幅変更による閉鎖ではジェスチャーを打ち切る。閉じた面の指が、移動先のパネルまで閉じてはいけない。
- **開いた中身の器は幅で変える**。`sm`以上では板がその場で伸びる（`--player-h`へは足さない。足すと開閉のたびに本文が跳ねる）。`sm`未満では下端から立ち上がるDrawerにする。狭い幅でその場に伸ばすと、画面の3割を占める中身が下部ナビと本文の隙間へ押し込まれ、どこから来た面なのかも判らない。器の形そのものが違うのでCSSでは選べず、`useMediaQuery`が幅を読む。
- 開いている間は、常設の行から**同じ役目のものを外す**。常設の目盛り・速度・音量は終了遷移が完了してDOMが外れるまで戻さない。Drawerから広幅へ移るときも、Drawerが外れてから展開段と板の失敗通知を出す。固定時間のタイマーで推測せず、連続開閉・両方向の幅変更・動きを止める設定でも、操作・alert・参照先idを二重にしない。
- 展開状態は端末へ残さない。「今いじりたい」だけの意図で、次に開くときまで持ち越す性質ではない。閉じる操作でも畳む。
- 送り・戻しは、狭い幅の折りたたみ時だけ隠す。隠す代わりに**展開すればどの幅でも出す**ようにし、辿り着けなくなる道を作らない。
- 題名の位置は開閉で変えない。押した瞬間にその要素ごと作り直されると、focusが本文の先頭へ落ちる。畳むときは**畳む前にfocusを題名へ移す**（開いた中身にfocusがあると、消えた瞬間に行き先を失う）。Escapeは畳むだけで音は止めず、板の外へも伝えない（記事一覧の選択解除を巻き添えにしない）。
- 展開時に操作列を中央へ据えるための空きは、**縮む余地を残す**。320pxではおもり・操作列・開閉ボタンの合計が板を38px超えるので、縮まないと開閉ボタンが板の外へ出る。中央に据えることより収まることを優先し、幅が足りるところでは中央へ戻る。支援する最も狭い幅（320px）は視覚回帰が固定する。
- 折りたたみ時の目盛りは、**置き場所を幅で変える**。`sm`以上では、それまで時刻の文字だけが座っていた題名の下の行をそのまま実目盛りへ格上げし、経過を左端・残りを負の符号で右端へ置く。行を増やさないので板の高さは変わらない。`sm`未満では同じ要素を板の上端の縁へ逃がし、時刻は下の行が文字で引き受ける。狭い幅の題名の列は100px前後しかなく、両端に時刻を置くと帯が30pxまで縮んで実質掴めないため。要素はどちらも同じ1つで、`absolute`か否かだけをCSSが切り替える（2つ置くと位置の購読も目盛りも二重になる）。
- 縁に置くときは、**見えるものだけを板の形に切り、掴み代は切らない**。板は`overflow`を切らず、帯が自分を`clip-path: inset(0 0 -200px 0 round …)`で切る。切る枠の上の角だけが板と同じ半径で丸まるので、帯は縁の曲がりまで辿って消える。当たり判定を持つ`input`は全幅のまま残すので、**板の角そのものを押しても先頭・末尾へ着く**（実測: 320〜1440pxで左端→0秒・右端→総時間・中央→ちょうど半分）。`overflow-hidden`で切ると丸めがヒットテストにも効き、端が掴めなくなる（実測: 左端+1pxのクリックが無反応、+12pxから効き始めた）。掴み代は帯よりずっと広く取るが、**縁では常設の行の上余白と同じ16pxに留める**。幅いっぱいに広がる帯なので、これより厚いと再生や閉じるの上端を覆い、そこを押すとシークしてしまう（実測: 20pxのとき再生ボタンの上8px・閉じるの上4pxが覆われた）。行の中では20px、開いた中身では24px。
- **`<input type=range>`のつまみは幅を0にし、見えるつまみは自前で置く**。幅を持たせると、UAはつまみが端から食み出さないよう`幅 − つまみ幅`の範囲でしか動かさない。塗りは全幅に対する`transform: scaleX()`で描いているので、両端で「つまみ幅の半分」ずれる（実測: 14pxのつまみで、0%のとき点だけが7px右に浮き、100%のとき塗りが点を7px追い越す）。幅を0にすると値と位置の対応が全幅で1対1になり、`left: <割合>%`に置いた自前のつまみが塗りの先端とも指の位置とも必ず一致する。音量も同じ組み方にする。
- 面が出入りする動きは`--ease-apple`で揃える。既定の`ease-out`は終わり際が緩すぎて、指を離した後もしばらく動いて見える。立ち上がりを速くし終わりを一気に止めると、面が「重さを持って置かれた」ように読める。押した手応え（操作列・絵）も同じ緩急で返す。動きを止める設定では、いずれも位置だけが変わる。
- つまみを出すのは、縁から離れている場所だけ。縁に密着した帯では、つまみは掴み代の上下中央に来るので帯から5px下にぶら下がり、帯へ合わせて持ち上げると`overflow-hidden`が切る。縁に置くときは掴めることを「触れると帯が3px→5pxへ太る」動きで示し、掴んだ位置は塗りの先端が示す。
- 速度は巡回ボタンではなく選択にする。巡回だと狙った速度へ着くまで候補の数だけ押すことになり、今どこに居るかも押してみるまで判らない。消音は音量とは別に持つ（0へ絞って消すと、戻したときの音量が判らなくなる）。
- 音量は**面の広さで出し方を変える**。常設の1行ではアイコンだけを置き、押すとその場へ帯が重なって開く。帯を常に並べると、最も長く取りたい題名から80px以上を奪う。合わせるのは稀な操作で、題名を読むのは常時なので、常設の幅は題名へ回す。開いた中身のように幅がある面では、開く操作を挟まず帯を並べる。
- `timeupdate`は毎秒数回届く。購読するのは目盛りと時刻表示だけに閉じ、操作ボタンは「鳴っているか」、一覧の行は「その番組が鳴っているか」と「その番組の再生記録」だけを購読する。一覧全体で`isPlayingAtom`を購読すると、1回の再生/停止で全行が描き直される。予算は`player-bar.render-count.test.tsx`が固定していて、ナビゲーションと操作列は位置が動いても0回。展開段は絵・題名・速度・音量を抱えるので、**開いたまま聴き続ける場合**も同じ予算を別に固定する。
- `backdrop-filter`が読むのは、その面より**後ろ**に描かれたものだけ。毎秒動くものは必ずglassの面の**子（前面）**へ置く。目盛り・時刻・再生中の波はいずれも板の前面なので、滲みの再計算を呼ばない。逆に、動くものをglassの面と同じか後ろの階層で重ねると、その都度この面まで焼き直される。下部ナビ（`z-20`）は再生バー（`z-30`）より後ろなので巻き込まれないが、ナビ自身を透かすと滲みが読むのは本文になり、スクロールのたびに帯全体を計算し直すので不透明のままにする（[ADR-0099](adr/0099-layered-glass-player-surface.md)）。
- 進んだ量は背景のグラデーションではなく`transform: scaleX()`で示す。グラデーションだと帯そのものを毎回描き直すことになる。再生中の波も振幅を`transform`で動かし、`height`は動かさない（毎フレームレイアウトからやり直しになる）。
- 押してから鳴り始めるまでの間を、失敗と区別して伝える。音声はGateway経由でS3からstreamされるので間が空くが、その間`playbackStatus`は既に`playing`で、ボタンだけが一時停止の形になり何も聞こえない。`waiting`/`playing`から導いた待ち状態は、**折りたたみ時は経過時間と同じ1行を入れ替えて**出す。並べて置くと、320pxでは題名の列(102px)へ137px入れることになり再生ボタンへ重なる。待っている間は位置も動かないので、入れ替えても失うものが無い。再生ボタンには`aria-busy`を付け、外周に1本回す（字で言うと操作列の幅が伸び縮みし、押し所が動く）。鳴らせなかった場合は`alert`で伝え、同じ行から再試行させる。この行は板の高さそのものを変えるので、`--player-h`の確保も**出ている間だけ厚くする**。確保しないと、板より手前に浮く回線切れの案内がちょうどこの行へ重なり、理由もやり直す道も読めなくなる（実測: 390pxで案内615..644が失敗の行606..651を覆う）。同じURLを代入し直しても要素は取りに行かないので`load()`を明示し、戻る位置は端末の記録から取る（失敗時の`currentTime`は0へ落ちていることがある）。
...
候補の記事を先に評価する。購読が0件・停止中でもアーカイブ済みの記事があれば手動生成できる。候補がなく追加ページが残る場合は「もっと読み込む」を残し、一覧を読み切る前に入力元なしと判断しない。

| 空の理由 | 案内・操作 |
| --- | --- |
| 検索中 | 直前の候補と検索中表示を維持。結果0件と確定しない |
| 検索結果0件 | 検索語をクリア |
| 購読なし | RSSフィードを追加 → `/subscriptions` |
| 全購読が停止中 | 購読を再開 → `/subscriptions` |
| 有効な購読の最新jobがqueued/processing | 取り込み中、同期状況を確認、候補を再読み込み |
| 最新jobがfailed、または一部記事の取得に失敗 | 失敗案内、購読画面で理由を確認して再同期 |
| 同期中でなく候補なし | 待つだけで解消すると約束せず、購読管理と再読み込みを案内 |
| 購読・同期状態の取得失敗 | 入力元なしと誤判定せず、再確認を表示 |

購読・同期queryは空のダイアログ内の独立した接続コンポーネントで読み、設定カードと同じquery key/cacheを使う。生成ステータス全体をsuspend・再描画させない。有効な購読ごとの最新jobだけで分類し、削除済み・停止中のfeedや過去の失敗は判断から除く。同期中と失敗が混在する場合は取り込み中を優先し、詳細は購読画面で確認する。再読み込みは候補と必要な購読・同期状態を取り直す。
...
| 再生バー                 | 下端に浮くglassの板 | 下部ナビの上に浮く | 常設: 絵、題名、目盛り、再生/一時停止、15秒戻し/30秒送り。64rem以上は速度・音量Popoverも常設。開くと: 大きい目盛り、残り、速度、音量、原稿への道（広い幅は板が伸び、狭い幅はDrawer） |
...
保存MarkdownはGFM、math、Mermaid、Obsidian/GitHub型callout、`@[card]`、`@[embed]`、code fence metadataを扱う。code言語は明示属性、filename、shebang/modeline、閾値付きoffline検出の順に決める。Webはcalloutを`@r4ai/remark-callout`で描画し、元にない見出しとparser由来の空段落を除去する。明示タイトル・折りたたみ・本文の記号は保持する。embedはHTTPS provider allowlist、sandbox、`no-referrer`を満たす場合だけ自動ロードする。sandboxの権限はprovider単位で宣言し、必要な物だけを与える。動画プレイヤーやスライドはJavaScriptなしでは何も描けないため`allow-scripts`を与えるが、`allow-same-origin`は型で表現できないようにして決して与えない（両方揃うとiframeが自分でsandbox属性を外せる）。許可リストに載らないhostnameは、URLがどれだけ安全に見えてもiframeにせずリンクへ落とす。

Zennの遅延埋め込みはProfileのselector/意味対応を使い、共有Ruleが`iframe[data-content]`のURI符号化されたリンク先・Mermaidソースと`embed-katex`のTeXを復元する。リンクカードは既存の`@[card]`、GitHub埋め込みは元URLへのfallbackを持つ`@[embed]`へ変換する。`details/summary`はsanitize済みの構造を残し、内部の本文は共通Markdownへ変換するため、入れ子のcallout・code・mathも保持する。summary内の数式は描画側のmath HTMLへ正規化する。重複リンクの除去は同じURLを持つ`display:none`の代替リンクに限定し、通常リンクの説明文は保持する。カードは取得時に既存のDNS固定fetchでOGPを補完し、`@[card]`のlink titleへ`link-card:v1:`付きmetadataを保存する。Webはタイトル・説明・favicon・ドメインとサムネイルを描画し、取得失敗時はURLへ退避する。最大12種類・同時3件・補完4秒・各HTML最大1 MiBに制限し、成否を`archive.link_card`で観測する（[ADR-0104](adr/0104-archive-link-card-metadata.md)）。
...
- [ADR-0090 LLMのsource identityを位置でsnapshotへ固定する](adr/0090-bind-generated-source-identity-by-position.md)
