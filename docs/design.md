...
Zennの遅延埋め込みはProfileのselector/意味対応を使い、共有Ruleが`iframe[data-content]`のURI符号化されたリンク先・Mermaidソースと`embed-katex`のTeXを復元する。リンクカードは既存の`@[card]`、GitHub埋め込みは元URLへのfallbackを持つ`@[embed]`へ変換する。`details/summary`はsanitize済みの構造を残し、内部の本文は共通Markdownへ変換するため、入れ子のcallout・code・mathも保持する。summary内の数式は描画側のmath HTMLへ正規化する。重複リンクの除去は同じURLを持つ`display:none`の代替リンクに限定し、通常リンクの説明文は保持する。カードは取得時に既存のDNS固定fetchでOGPを補完し、`@[card]`のlink titleへ`link-card:v1:`付きmetadataを保存する。Webはタイトル・説明・ドメインとサムネイルを描画し、取得失敗時はURLへ退避する。最大12種類・同時3件・補完4秒・各HTML最大1 MiBに制限し、成否を`archive.link_card`で観測する（[ADR-0104](adr/0104-archive-link-card-metadata.md)）。
