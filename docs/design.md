...
- 一覧はopaque cursor、`limit` 1..100、安定順序、filterに束縛する。`totalCount`は初期契約に入れない。
  ...
  - 手動生成の候補は初回・検索・追加ページとも`sort=newest`（新着順）で取得し、アーカイブ済みの記事だけを表示する。「上から一括選択」は表示済みの先頭から最大20件を選ぶ。現行APIは`newest` / `oldest`のみ対応し、スコアの有無は順序へ影響しない。旧ADR-0021/0022の推薦順は現行実装で未対応のため案内しない。
