...
AI記事補完のキュー、結果、タグ、日次使用量はすべてowner単位である。workerはownerごとに`CONTENT_ENRICH_DAILY_LIMIT`の使用量を読み、枯渇したownerだけをskipして次のownerを処理する。成功確定と`(owner_id, local_date)`使用量の加算は同じtransactionで行い、開発用リセットも認証actorのownerだけを対象にする。詳細は[ADR-0063](adr/0063-scope-enrichment-daily-budget-by-owner.md)を正本とする。
...
- 読み込み中の骨組みは、実際に待っている範囲だけに掛ける。記事一覧の表示境界(`Panel`)は記事行だけを包み、ヘッダーは境界の外に置く。パネルごと包むと、絞り込みを変えて取り直すたびに検索欄と状態タブまで骨組みへ差し替わり、打った直後に打ち直せなくなる。骨組みは待っている中身と同じ骨格・同じ余白で描き、届いた瞬間に高さが飛ばないようにする。
...
- リーダーは本文の見出しから目次を作る。幅に余裕がある時（`xl`）は追従する右レール、それ未満は本文の前に置く。`<aside>`にはしない（AppShellのサイドバーが既にcomplementaryランドマークを持つため、axeの`landmark-unique`に触れる）。同じ名前の`nav`が2つ露出するのも同じ理由で避け、幅による出し分けは`display:none`で行う。字下げは絶対レベルではなく最も浅い見出しからの相対の深さで決め、2階層までに留める。GFM脚注の`sr-only`な「Footnotes」見出しは目次に載せない。
- 目次は開閉できる。開閉はBase UIのCollapsibleで、高さは`--collapsible-panel-height`へのtransitionで補間する（`auto`は補間できず、JSで高さを測る実装も要らない）。畳んだ中身は`hidden="until-found"`でDOMへ残し、ページ内検索から見つかるようにする。右レールは開いたまま、本文の前に置く方は畳んで始める（開いたままだと記事を開くたびに本文がその分だけ下へ押される）。
- 目次の追従（sticky）は器の`div`が持ち、`self-start`で高さを中身へ戻す。スクロール領域はflexなので、子は既定で領域の高さまで引き伸ばされる。伸びた枠の中でstickyにすると追従できる範囲が1画面ぶんで尽きるため、リーダー本体の枠も`self-start`にして本文の長さいっぱいまで動けるようにする。この回帰は実際に長い本文を送らないと見えないので、e2eで確かめる。
- 現在地は面ではなく軸で示す。目次の項目は左の罫を軸に並べ、読んでいる節だけがその罫の色を持つ。行の背景を塗ると面が増えて本文と競う。
