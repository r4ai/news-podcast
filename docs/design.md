...
- 板は「常設1行」と「展開段」に分ける。常設は**絵・題名・経過・15秒戻し/再生/30秒送り・開閉・閉じる**だけ。大きい目盛り・残り時間・速度・音量・原稿への道は展開段が持つ。常時2段にすると、聴いていない時間まで本文が2行ぶん削られる。展開段は板の上へ伸びる覆いなので`--player-h`へは足さない（足すと開閉のたびに本文が跳ねる）。
- 展開状態は端末へ残さない。「今いじりたい」だけの意図で、次に開くときまで持ち越す性質ではない。閉じる操作でも畳む。
- 送り・戻しは、狭い幅の折りたたみ時だけ隠す。隠す代わりに**展開すればどの幅でも出す**ようにし、辿り着けなくなる道を作らない。
- 開閉ボタンの位置は段をまたいで変えない。押した瞬間にその要素ごと作り直されると、focusが本文の先頭へ落ちる。Escapeは畳むだけで音は止めず、板の外へも伝えない（記事一覧の選択解除を巻き添えにしない）。
...
- 縁に置くときは**両端を角の丸みの内側へ収める**。`overflow-hidden`の丸めは描画だけでなくヒットテストにも効くので、端まで伸ばすと「先頭へ戻す」「末尾へ飛ぶ」が掴めない（実測: 左端+1pxのクリックが無反応、+12pxから効き始める）。掴み代は帯よりずっと広く取る（縁では20px、行の中では20px、展開段では24px）。
...
- 速度は巡回ボタンではなく選択にする。巡回だと狙った速度へ着くまで候補の数だけ押すことになり、今どこに居るかも押してみるまで判らない。音量は押して開くpopoverにせず、展開段へ常設する。鳴らしながら合わせるものなので、開く操作を挟むと合わせている間ずっと本文が覆われる。消音は音量とは別に持つ（0へ絞って消すと、戻したときの音量が判らなくなる）。
...
| 再生バー                 | 下端に浮くglassの板 | 下部ナビの上に浮く | 常設: 絵、題名、経過、再生/一時停止、15秒戻し/30秒送り。展開段: 大きい目盛り、残り、速度、音量、原稿への道 |
