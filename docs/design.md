...
- 折りたたみ時の目盛りは板の上端の縁そのものに置き、幅いっぱいを掴めるようにする。ただし**両端は角の丸みの内側へ収める**。`overflow-hidden`の丸めは描画だけでなくヒットテストにも効くので、端まで伸ばすと「先頭へ戻す」「末尾へ飛ぶ」が掴めない（実測: 左端+1pxのクリックが無反応、+12pxから効き始める）。内側へ寄せることで、目盛りの左端が値0、右端が総時間と1対1で対応する。掴み代は帯よりずっと広く取る（20px。題名の行箱の上端へ4px重なるが、字はそこまで伸びない）。**つまみは出さない**。UAのつまみは掴み代の中央へ置かれるので、縁に密着した帯からは5px下にぶら下がり、帯へ合わせて持ち上げると板の`overflow-hidden`が切る。掴めることは触れると帯が3px→5pxへ太る動きが示し、掴んだ位置は塗りの先端が示す。展開時は掴み代を24pxまで広げ、つまみも出す（縁から離れているので合わせられる）。経過を左端・残りを負の符号で右端へ置く。同じ書式で左右に並べると、どちらがどちらか読むまで判らない。
