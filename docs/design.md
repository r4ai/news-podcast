...
- 折りたたみ時の目盛りは板の上端の縁そのものに置き、幅いっぱいを掴めるようにする。ただし**両端は角の丸みの内側へ収める**。`overflow-hidden`の丸めは描画だけでなくヒットテストにも効くので、端まで伸ばすと「先頭へ戻す」「末尾へ飛ぶ」が掴めない（実測: 左端+1pxのクリックが無反応、+12pxから効き始める）。内側へ寄せることで、目盛りの左端が値0、右端が総時間と1対1で対応する。掴み代は帯より広く取り、つまみは触れた時だけ出す（3pxの帯の上に丸を常駐させると、帯そのものが掴む対象に見えなくなる）。展開時は帯を太くし、経過を左端・残りを負の符号で右端へ置く。同じ書式で左右に並べると、どちらがどちらか読むまで判らない。
