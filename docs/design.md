...
記事replay proxyは`article.replay_proxy`を成功・失敗の両方で記録し、属性は`object_kind`（`replay`/`asset`）、`outcome`、`duration_ms`だけに限定する。署名URL、object key、元記事URL、owner/snapshot IDは記録しない。
