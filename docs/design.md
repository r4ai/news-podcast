...
- 板は「常設1行」と「開いた中身」に分ける。常設は**絵・題名・目盛り・15秒戻し/再生/30秒送り・閉じる**だけ。大きい目盛り・速度・音量・原稿への道は開いた中身が持つ。常時2段にすると、聴いていない時間まで本文が2行ぶん削られる。
...
- 下端から立ち上がる面（Drawer）の掴み代は、**引き下げて閉じられるようにする**。掴み代を描くだけで手を付けないと、引けそうに見えて引けない面になる。押しても閉じるようにし、動きと終わりは窓で受ける（掴み代の上だけで受けると、指が外へ出た瞬間やブラウザが自前の引きずりを始めた瞬間に引いた量を見失う）。
...
- 開いている間は、常設の行から**同じ役目のものを外す**。板がその場で伸びる幅では、常設の目盛り・速度・音量を出さない（開いた中身が帯として持つ）。残すと同じ名前の操作が画面に2つ並ぶ。
...
| 再生バー                 | 下端に浮くglassの板 | 下部ナビの上に浮く | 常設: 絵、題名、目盛り、再生/一時停止、15秒戻し/30秒送り。開くと: 大きい目盛り、残り、速度、音量、原稿への道（広い幅は板が伸び、狭い幅はDrawer） |
