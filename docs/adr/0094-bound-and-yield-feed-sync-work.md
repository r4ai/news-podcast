...
| Cycle | 最大6 claim。DBに実際の待機jobがある時だけ100ms後に継続。完了・失敗feedの自動再投入は終端から最低5分空け、未処理feedを優先 |
...
| Processing | Feed失敗/deadline | Failed | snapshot・記事別件数を保持、5分後の再試行時に再開。feed障害は記事失敗件数へ加算しない |
...
| Storage | `ready_sequence`、`ready_at`、`continuation_json`、`item_error` migration。既存jobも再開可能 |
...
- レビュー指摘もRedから検証：6 feedの一時障害で即時retryしない、復旧時にfeed障害を記事失敗へ残さない、100 feedのmetadata読込を298 SELECTから1 SELECTへ削減。
