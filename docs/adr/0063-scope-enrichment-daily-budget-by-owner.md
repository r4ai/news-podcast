...
AI記事補完の日次上限は`ownerId + UTC localDate`ごとに評価する。成功完了時刻を1回取得し、そのUTC日付へ利用量を加算する。workerの残量判定、状態表示、許可されたリセットも同じowner境界で実行する。日付をまたぐ処理は開始日ではなく完了日に属する。リセットを誰に許可するかは[ADR-0076](0076-fail-closed-enrichment-budget-reset.md)で定める。
