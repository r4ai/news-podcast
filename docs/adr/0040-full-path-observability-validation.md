...
- Grafana provisioningはhost directoryをcontainer directoryへbindせず、provisioning YAMLと各Dashboard JSONを個別ファイルとしてmountする。Dashboard/Prometheus/Loki/Tempoにhealthcheckを付ける。
...
- Dashboard UID、datasource health、Collector accepted/refused/export failure、Browser proxy、実依存endpointを`pnpm observability:smoke`で検証する。trace IDを渡した場合はTempo/Loki/Prometheusの相関も検証する。
...
| Data and storage | N/A —既存volumeを再起動で保持 | Proposed until E2E | Compose volume definitions |
...
| Tests and operations | static validation + API/endpoint smoke + final restart E2E | Proposed until E2E | `pnpm observability:validate`、`pnpm observability:smoke` |
...
- 最終証拠: `pnpm observability:smoke`の出力、再起動コマンド、`docker compose ps`、実trace ID、失敗注入の結果をこのADRまたは実装報告へ追記する。
