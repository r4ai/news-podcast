...
- 保存領域はorigin単位なので、**明示logoutのserver成功後、または失効sessionでログイン画面に着いた時点で番組と再生記録を捨てる**。明示logoutではaudioを停止・unloadしてからdocumentを置き換える（ADR-0078）。同じブラウザで別の利用者がログインしたときに、前の利用者の番組名や続きが復元されるのを防ぐ。再生速度は個人を表さない端末の設定なので残す。
- 別タブの書き込みを取り込むのは、その値が単独で意味を持つもの（再生記録・速度）だけ。「今どの番組を鳴らしているか」はそのタブの`<audio>`と対でしか意味を持たないので取り込まない。取り込むと、鳴っている音は前の番組のまま見出しだけ差し替わり、その位置を別の番組の記録として保存してしまう。
- OSから届くのは**命令**であって切り替えではない。再生と停止は別々の書き込み専用atomにし、同じ命令が二度届いても状態が反転しないようにする（画面のボタンだけが、その2つを使って切り替える）。

### 4. ライブラリは一覧と原稿の2ペインにする

- 左が番組一覧（日付で括った行だけ）、右が詳細（原稿を主、出典を右レール）。選択は`?episode=`でURLが正本。
- スクロールしているのは詳細の外枠で、`key`で差し替わるのは中身だけ。番組を切り替えたら枠の位置を頭へ戻す。戻さないと、長い台本を読んだ後に隣の番組を開いたとき、題名も再生ボタンも画面の外から始まる。
- 続きの取得失敗は`Panel`まで上げない（1ページ目は既に描けている）。行の末尾で理由を出し、同じ場所から再試行させる。
...
- 端末に再生記録が残る（`player.progress` / `player.track` / `player.rate`）。上限200件で古いものから捨て、logoutまたは失効sessionで破棄するが、ログアウトせずに端末を明け渡した場合は次の利用者から見える。
...
| 実行/配備 | N/A | Done | N/A |
| 認証/セキュリティ | 音声はsame-originのcookie認証のまま（ADR-0055）。ログイン画面で端末の再生記録を破棄 | Done | `apps/web/src/routes/login/index.tsx` |
