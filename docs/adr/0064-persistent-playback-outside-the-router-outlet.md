...
- 保存領域はorigin単位なので、**ログイン画面に着いた時点で番組と再生記録を捨てる**。同じブラウザで別の利用者がログインしたときに、前の利用者の番組名や続きが復元されるのを防ぐ。再生速度は個人を表さない端末の設定なので残す。
...
- 端末に再生記録が残る（`player.progress` / `player.track` / `player.rate`）。上限200件で古いものから捨て、ログイン画面で破棄するが、ログアウトせずに端末を明け渡した場合は次の利用者から見える。
