...
### 3. 段は「常設1行」と「展開段」に分ける

- 常設は1行だけ。**絵・題名・経過・トランスポート・開閉**に限る。本文から恒久的に奪う高さを`6rem`→`4.75rem`へ減らす。
...
- Escapeは畳むだけで、音は止めない。板の外へは伝えない（記事一覧の選択解除を巻き添えにしない）。
...
- 板の上端の縁に置く目盛りは、**両端を角の丸みの内側へ収める**。`overflow-hidden`による角の丸めは描画だけでなく**ヒットテストにも効く**ので、縁いっぱいに伸ばすと両端が掴めない。実測では、寄せる前は左端+1pxのクリックが無反応で+12pxから効き始め、右端も同様だった。寄せた後は左端で0秒、右端で総時間、中央でちょうど半分へ着く。
