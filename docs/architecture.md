...
- ownerごとのactive job（`queued` / `running` / `retrying`）は最大1件。同一scope・`Idempotency-Key`の再送は先にreplayし、異なるmanual/retry要求はactive job参照付き409、scheduled要求はdueのまま次tickへ延期する。DB triggerを最終的な原子ガードとする（[ADR-0091](adr/0091-bound-owner-active-episode-jobs.md)）。
