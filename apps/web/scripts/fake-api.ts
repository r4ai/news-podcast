  ...
  return [
    {
      ...
      script: [
        ...
        ...Array.from(
          { length: 20 },
          (_, index) =>
            `補足の${index + 1}点目です。実際に計測した数値と、そこから読み取れる運用上の注意点を順に説明します。`
        ),
    ...
    },
    {
      ...
      // 長い台本。詳細ペインが実際にスクロールする状態でないと、番組を
      // 切り替えたときの位置戻しをe2eで確かめられない。
      script: [
        ...
        ...Array.from(
          { length: 24 },
          (_, index) =>
            `第${index + 1}の話題です。配信基盤の入れ替えにあたって検討した点と、実際に計測した結果を順に説明します。読み上げの尺を確かめるための段落です。`
        ),
        "今週は以上です。詳しくは出典の記事をご確認ください。",
      ].join("\n\n"),
      sources: [
        {
